...
def process(start_time, input_path, output_path, model_name="u2net",
    ...
    return image
    # return response(time.time() - start_time, "", "", 1, "", output_path)
...
def cli(start_time, image):
    ...
    input_path = ".\\docs\\imgs\\tes\\1.jpg"
    output_path = ".\\docs\\imgs\\output\\tes\\"
    model_name = "u2net"
    ...
    if model_name == "test":
        ...
        return process(start_time, image, output_path, model_name, preprocessing_method_name,
                       postprocessing_method_name)
...
def upload():
    ...
    try:
        ...
        if file:
            ...
            result = cli(time.time(), image2)
            with io.BytesIO() as output:
                result.save(output, format="PNG")
                print(time.time() - start_time)
                return flask.send_file(io.BytesIO(bytearray(output.getvalue())), mimetype='image/png')
